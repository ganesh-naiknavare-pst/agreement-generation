import { ActionIcon, Table, Text, Center, Badge, Tooltip } from '@mantine/core';
...
export function Agreements() {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', width: '100%', height: '100%' }}>
        <Table striped highlightOnHover withTableBorder verticalSpacing="md" horizontalSpacing={20}>
          <Table.Thead>
            <Table.Tr>
            ...
            {agreements.length > 0 ? (
              agreements.map((agreement) => (
                <Table.Tr key={agreement.id}>
                  <Table.Td style={{ textAlign: 'left' }}>{agreement.title}</Table.Td>
                  <Table.Td style={{ textAlign: 'left' }}>{agreement.ownerName}</Table.Td>
                  <Table.Td style={{ textAlign: 'left' }}>{agreement.tenantName}</Table.Td>
                  <Table.Td style={{ textAlign: 'left' }}>{agreement.createdTime}</Table.Td>
